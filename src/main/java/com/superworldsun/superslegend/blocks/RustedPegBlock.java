...
public class RustedPegBlock extends Block {
    ...
    public RustedPegBlock(Properties pProperties) {
        super(pProperties);
    }
    ...
    {
        if (player.isHolding(ItemInit.MEGATON_HAMMER.get()) || player.isHolding(ItemInit.SKULL_HAMMER.get()))
        {
        ...
        {
            BlockPos currentPos = player.blockPosition();
            world.playSound(null, currentPos.getX(), currentPos.getY(), currentPos.getZ(), SoundEvents.ANCIENT_DEBRIS_BREAK, SoundSource.PLAYERS, 1f, 1f);
        }
