...
public class ClawshotEntity extends AbstractArrowEntity {
	...
	public void tick() {
		...
		if (getOwner() instanceof PlayerEntity) {
			...
			if (!level.isClientSide) {
				...
				//System.out.println(owner.getMainHandItem());
					...
					if (isPulling) { //Movement start
						...
						if(!motionUp) {
							...
							if(Minecraft.getInstance().options.keyUp.isDown()) {
								...
								double y = Math.sin(Math.toRadians(target.xRot));
								System.out.println(y);
								target.setDeltaMovement(Math.min(origin.xo-target.xo, 0.1), -y*pullSpeed, Math.min(origin.zo-target.zo, 0.1));
							}
							else {
								target.setDeltaMovement(Math.min(origin.xo-target.xo, 0.1), 0, Math.min(origin.zo-target.zo, 0.1));
							}
							target.hurtMarked = true; //Make motion works, this is necessary.
							//System.out.println("UP");
							//System.out.println(origin.xo);
						}
					}
	...
	public static void onPlayerTick(PlayerTickEvent event) {
		...
		if (true) {
			double maxRange = 15;
			List<ClawshotEntity> entities = player.level.getEntitiesOfClass(ClawshotEntity.class, new AxisAlignedBB(player.blockPosition().offset(-maxRange, -maxRange, -maxRange), player.blockPosition().offset(maxRange, maxRange, maxRange)));
			for(ClawshotEntity entity : entities) {
				if(entity.getOwner() == player) {
					if (entity.isPulling && !entity.motionUp) {
