...
public class GreenRing extends RingItem {
    public GreenRing(Properties properties) {
    ...
    public static void livingDamageEvent(LivingDamageEvent event)
    {
        ...
        if (event.getSource().getDirectEntity() instanceof Player)
        {
            ...
            ItemStack stack = CuriosApi.getCuriosHelper().findEquippedCurio(ItemInit.GREEN_RING.get(), player).map(ImmutableTriple::getRight).orElse(ItemStack.EMPTY);
    ...
    public void appendHoverText(ItemStack stack, Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
        tooltip.add(Component.literal("1.5x Sword Damage & Damage Reduced").withStyle(ChatFormatting.GREEN));
