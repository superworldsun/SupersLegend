...
public class RedLuckRing extends Item implements ICurioItem {

    public RedLuckRing(Properties p_i48487_1_) {
        super(p_i48487_1_);
    }
    ...
    public static void onLivingHurt(LivingHurtEvent event) {
        ...
        if (event.getEntityLiving() instanceof PlayerEntity) {
            ...
            ItemStack stack =
                    CuriosApi.getCuriosHelper().findEquippedCurio(ItemInit.RED_LUCK_RING.get(), player).map(
            ...
            if (!stack.isEmpty()) {
                ...
                if (event.getSource() == DamageSource.CACTUS || event.getSource() == DamageSource.SWEET_BERRY_BUSH
                        || event.getSource() == DamageSource.thorns(player) || event.getSource() == DamageSource.thorns(player.getEntity())
                        || event.getSource() == DamageSource.thorns(event.getEntity()))
                {
    ...
    /*if (!stack.isEmpty()) {
        ...
        if (event.getEntityLiving() instanceof GuardianEntity) {
            GuardianEntity guardian = (GuardianEntity) event.getEntityLiving();
            {
                if (event.getSource() == DamageSource.thorns(guardian.getEntity())) {
                    event.setAmount(event.getAmount() * 9);
                }
            }
        }
