...
public class MaskGoronmask extends Item implements IPlayerModelChanger, IEntityResizer, ICurioItem {
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        ItemStack stack0 = CuriosApi.getCuriosHelper().findEquippedCurio(ItemInit.MASK_GORONMASK.get(), event.player).map(ImmutableTriple::getRight).orElse(ItemStack.EMPTY);
        if (!stack0.isEmpty()) {
