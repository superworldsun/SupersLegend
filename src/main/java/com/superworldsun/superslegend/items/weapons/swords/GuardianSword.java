...
public class GuardianSword extends ItemCustomSword {
    ...
    public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, @NotNull List<Component> tooltip, @NotNull TooltipFlag flag) {
        if(!Screen.hasShiftDown()) {
            tooltip.add(Component.literal("A sword made from ancient technology").withStyle(ChatFormatting.AQUA));
            tooltip.add(Component.literal("[Hold Shift for Info]").withStyle(ChatFormatting.DARK_GRAY));
        }
        else if(Screen.hasShiftDown()) {
            tooltip.add(Component.literal("Can provide a damage buff when used with Ancient Armor set").withStyle(ChatFormatting.GRAY));
