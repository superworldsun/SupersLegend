...
public class ClawshotItem extends NonEnchantItem {
    ...
    */
    ...
    public void releaseUsing(ItemStack itemStack, World world, LivingEntity player, int remainingUseTicks) {
        ...
        world.playSound(null, currentPos.getX(), currentPos.getY(), currentPos.getZ(), SoundInit.CLAWSHOT_FIRE.get(), SoundCategory.PLAYERS, 1f, 1f);
        ...
        if (!world.isClientSide) {
            if (!HookModel.get((PlayerEntity) player).getHasHook()) {
                ...
                ClawshotEntity hookshot = new ClawshotEntity(EntityTypeInit.CLAWSHOT_ENTITY.get(), player, world);
