...
import com.superworldsun.superslegend.client.model.ClawshotModel;
...
@OnlyIn(Dist.CLIENT)
public class ClawshotRender extends EntityRenderer<ClawshotEntity> {
    ...
    public ClawshotRender(EntityRendererManager renderManagerIn) {
        super(renderManagerIn);
    }

    // Function that allows the rendering of the chain and the hook.
    @Override
    public void render(ClawshotEntity clawshotEntity, float yaw, float tick, MatrixStack matrixStack, IRenderTypeBuffer provider, int light) {
        ...
        PlayerEntity player = Minecraft.getInstance().player;
            ...
            if(player != null && !HookModel.get(player).getHasHook()) {
                ...
                float distanceX = (float) (startX - clawshotEntity.getX());
                float distanceY = (float) (startY - clawshotEntity.getY());
                float distanceZ = (float) (startZ - clawshotEntity.getZ());

                float message1 = distanceX * distanceX + distanceY * distanceY + distanceZ * distanceZ;
                boolean message = message1 < 2;
                if (!(message)) {
                    renderChain(distanceX, distanceY, distanceZ, tick, clawshotEntity.tickCount, matrixStack, provider, light);
                    renderSecondChain(distanceX, distanceY, distanceZ, tick, clawshotEntity.tickCount, matrixStack, provider, light);
                }

                renderHook(distanceX, distanceY, distanceZ, tick, clawshotEntity.tickCount, matrixStack, provider, light);

                matrixStack.popPose();
            }


        super.render(clawshotEntity, yaw, tick, matrixStack, provider, light);
    }
    ...
    @Override
    public ResourceLocation getTextureLocation(ClawshotEntity entity) {
        return TEXTURE;
    }
