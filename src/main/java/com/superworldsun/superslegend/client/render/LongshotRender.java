...
public class LongshotRender extends EntityRenderer<LongshotEntity> {
    ...
    public LongshotRender(EntityRendererManager renderManagerIn) {
        super(renderManagerIn);
    }

    // Function that allows the rendering of the chain and the hook.
    @Override
    public void render(LongshotEntity longshotEntity, float yaw, float tick, MatrixStack matrixStack, IRenderTypeBuffer provider, int light) {
        ...
        PlayerEntity player = Minecraft.getInstance().player;
            ...
            if(player != null && !HookModel.get(player).getHasHook()) {
                ...
                float distanceX = (float) (startX - longshotEntity.getX());
                float distanceY = (float) (startY - longshotEntity.getY());
                float distanceZ = (float) (startZ - longshotEntity.getZ());

                float message1 = distanceX * distanceX + distanceY * distanceY + distanceZ * distanceZ;
                boolean message = message1 < 2;
                if (!(message)) {
                    renderChain(distanceX, distanceY, distanceZ, tick, longshotEntity.tickCount, matrixStack, provider, light);
                    renderSecondChain(distanceX, distanceY, distanceZ, tick, longshotEntity.tickCount, matrixStack, provider, light);
                }
                renderHook(distanceX, distanceY, distanceZ, tick, longshotEntity.tickCount, matrixStack, provider, light);

                matrixStack.popPose();
            }


        super.render(longshotEntity, yaw, tick, matrixStack, provider, light);
    }
