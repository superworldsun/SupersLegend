...
public class ShadowRenderer<T extends ShadowTileEntity> extends TileEntityRenderer<T>
{
	...
	{
		BlockState blockState = te.getAppearance();
		...
		if (blockState.getRenderShape() == BlockRenderType.MODEL)
		{
			...
			for (RenderType renderType : RenderType.chunkBufferLayers())
			{
				...
				{
					renderBlock(world, blockPos, blockState, matrixStack, buffer, renderType);
				}
			}
	...
	private void renderBlock(World world, BlockPos blockPos, BlockState blockState, MatrixStack matrixStack, IRenderTypeBuffer renderTypeBuffer, RenderType renderType)
	{
		...
		blockRenderer.getModelRenderer().renderModel(world, model, blockState, blockPos, matrixStack, vertexBuffer, false, new Random(), seed, overlay, EmptyModelData.INSTANCE);
	}
}
